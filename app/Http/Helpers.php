...
if (!function_exists('sendPasswordToUser')) {
    function sendPasswordToUser(User $user, $password, $canal)
    {
        switch ($canal) {
            case 'email':
                $data = [
                ...
                try {
                    ...
                    Log::info("email sent to ". $user->emai);
                    Log::info($user);
                } catch (Exception $ex) {
                ...
                break;
            case 'phone':
                SmsUtility::send_reset_password($user,$password);
                break;

            default:
                Log::warning("canal should be either email or phone. canal provided : ".$canal);
                break;
        }
    }
}

if (!function_exists('sendInvoiceByEmail')) {
    function applyCouponCaculation($couponCode, $amount)
    {
        $coupon = Coupon::where('code', $couponCode)->first();
        if(!empty($coupon)){
            if($coupon->type == 'subscription_base'){
                $discountAmount = 0;
        ...
        return null;
        // TODO add discount caclulation for other coupon types
    }
}
...
if (!function_exists('sendInvoiceByEmail')) {
    function sendInvoiceByEmail( CustomerPackagePayment $packagePayment  )
    {
        $user = $packagePayment->user;
        $packageDetails = $packagePayment->customer_package;
        Log::info($packageDetails);
        Log::info($user);
        $data = [
            "user_name" => $user->name,
            "email" => $user->email,
            ...
            "date" => $packagePayment->date_invoice,
            "invoice_id" => $packagePayment->id,
            "qr_code" => $packagePayment->qr_code,
            "vat_val" => $packagePayment->vat_total
        ];
        $to = $user->email;
...
if (!function_exists('uploaded_asset')) {
    function uploaded_asset($id)
    {
        if (($asset = \App\Models\Upload::find($id)) != null) {
            return !empty($asset->file_name) ? my_asset($asset->file_name): static_asset('assets/img/placeholder.jpg') ;
        }
...
if (!function_exists('getCurrentDateInUTCFormat')) {
    ...
    function getCurrentDateInUTCFormat() {
        ...
        $formattedDate = $currentDateTime->format('Y-m-d\TH:i:s.v\Z');
...
if (!function_exists('isSubscribedUser')) {
    ...
    function isSubscribedUser(User $user) {
        if(empty($user)) return false;

        $now = Carbon::now();
        if($user->end_sub_date > $now && $user->start_sub_date <= $now && $user->customer_package_id !=null){
            return true;
        }
        return false;
    }
