...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if ($request->payment_option != null) {
            ...
            if ($request->session()->get('combined_order_id') != null) {
                ...
                if (in_array($request->input('payment_option'), $paymentMethods)) {
                    $combined_order = CombinedOrder::findOrFail($request->session()->get('combined_order_id'));
                    $vat = floatval($combined_order->grand_total) * 0.15;
                    $qrCode = Zatca::sellerName(get_setting("owner_company_name"))
                            ...
                            ->totalWithVat($combined_order->grand_total+$vat)
                            ->vatTotal($vat)
                            ->toBase64();
                    return redirect()->route("order-payment",[
                        "amount" => $combined_order->grand_total+$vat,
                        "qr_code" => $qrCode, //to generate qr with zakatka
                        "vat_total" => $vat,
                        "user_id" => auth()->user()->id,
                        "card_type" => $request->input('payment_option'),
    ...
    public function apply_coupon_code(Request $request)
    {
        ...
        if ($coupon != null) {
            ...
            if (strtotime(date('d-m-Y')) >= $coupon->start_date && strtotime(date('d-m-Y')) <= $coupon->end_date) {
                if ($coupon->type == 'subscription_base') {
                    ...
                    $response_message['coupon'] = $coupon;
                ...
                if (CouponUsage::where('user_id', Auth::user()->id)->where('coupon_id', $coupon->id)->first() == null) {
                    ...
                    if ($coupon_discount > 0 && $coupon->type !== 'subscription_base') {
        ...
        $returnHTML = null;
        if(!empty($coupon) && $coupon->type !== 'subscription_base'){
            $carts = Cart::where('user_id', Auth::user()->id)
            ->get();
            $shipping_info = Address::where('id', $carts[0]['address_id'])->first();
