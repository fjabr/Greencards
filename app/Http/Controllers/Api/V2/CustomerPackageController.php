...
{
    public function customer_packages_list(Request $request)
    {
        $customer_packages = CustomerPackage::where("hidden",0);
        if($request->input("source") === 'flutter_app'){
            $customer_packages = $customer_packages->where("hide_in_greencart",0)->get();
        }else{
            $customer_packages = $customer_packages->get();
        }
