@extends('seller.layouts.app')

@section('panel_content')

    <div class="aiz-titlebar mt-2 mb-4">
      <div class="row align-items-center">
        <div class="col-md-6">
            <h1 class="h3">{{ translate('Shop Settings')}}
                <a href="{{ route('shop.visit', $shop->slug) }}" class="btn btn-link btn-sm" target="_blank">({{ translate('Visit Shop')}})<i class="la la-external-link"></i>)</a>
            </h1>
        </div>
      </div>
    </div>


    <div id="branches" data-obj="{{ json_encode($branches) }}"></div>


    {{-- Basic Info --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Basic Info') }}</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="shop_id" value="{{ $shop->id }}">
                @csrf
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Shop Name') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Shop Name')}}" name="name" value="{{ $shop->name }}" required>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Shop Name | Arabic') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Shop Name | Arabic')}}" name="name_ar" value="{{ $shop->name_ar }}" required>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Shop Logo') }}</label>
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image">
                            <div class="input-group-prepend">
                                <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse')}}</div>
                            </div>
                            <div class="form-control file-amount">{{ translate('Choose File') }}</div>
                            <input type="hidden" name="logo" value="{{ $shop->logo }}" class="selected-files">
                        </div>
                        <div class="file-preview box sm">
                        </div>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">
                        {{ translate('Shop Phone') }}
                    </label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Phone')}}" name="phone" value="{{ $shop->phone }}" required>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Country') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                            <select class="form-control" name="country_id" id="country_id" onchange="getCities('city_id')">
                            <option value="" >{{ translate("choose a Country") }}</option>
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}" {{($country->id ==  @$shop->city->state_id)?'selected':''}}>{{ $country->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('City') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <select class="form-control" name="city_id" id="city_id">
                            <option value="" >{{ translate("choose a City") }}</option>
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}" {{($city->id ==  @$shop->city_id)?'selected':''}}>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Shop Address') }} <span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Address')}}" name="address" value="{{ $shop->address }}" required>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Shop Address | Arabic') }} <span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Address')}}" name="address_ar" value="{{ $shop->address_ar }}" required>
                    </div>
                </div>
                @if (get_setting('shipping_type') == 'seller_wise_shipping')
                    <div class="row">
                        <div class="col-md-2">
                            <label>{{ translate('Shipping Cost')}} <span class="text-danger">*</span></label>
                        </div>
                        <div class="col-md-10">
                            <input type="number" lang="en" min="0" class="form-control mb-3" placeholder="{{ translate('Shipping Cost')}}" name="shipping_cost" value="{{ $shop->shipping_cost }}" required>
                        </div>
                    </div>
                @endif
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Meta Title') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Meta Title')}}" name="meta_title" value="{{ $shop->meta_title }}" required>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Meta Title | Arabic') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <input type="text" class="form-control mb-3" placeholder="{{ translate('Meta Title | Arabic')}}" name="meta_title_ar" value="{{ $shop->meta_title_ar }}" required>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Meta Description') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <textarea name="meta_description" rows="3" class="form-control mb-3" required>{{ $shop->meta_description }}</textarea>
                    </div>
                </div>
                <div class="row">
                    <label class="col-md-2 col-form-label">{{ translate('Meta Description | Arabic') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <textarea name="meta_description_ar" rows="3" class="form-control mb-3" required>{{ $shop->meta_description_ar }}</textarea>
                    </div>
                </div>
                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    @if (addon_is_activated('delivery_boy'))
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0 h6">{{ translate('Delivery Boy Pickup Point') }}</h5>
            </div>
            <div class="card-body">
                <form class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="shop_id" value="{{ $shop->id }}">
                    @csrf

                    @if (get_setting('google_map') == 1)
                        <div class="row mb-3">
                            <input id="searchInput" class="controls" type="text" placeholder="{{translate('Enter a location')}}">
                            <div id="map"></div>
                            <ul id="geoData">
                                <li style="display: none;">{{ translate('Full Address') }}: <span id="location"></span></li>
                                <li style="display: none;">{{ translate('Postal Code') }}: <span id="postal_code"></span></li>
                                <li style="display: none;">{{ translate('Country') }}: <span id="country"></span></li>
                                <li style="display: none;">{{ translate('Latitude') }}: <span id="lat"></span></li>
                                <li style="display: none;">{{ translate('Longitude') }}: <span id="lon"></span></li>
                            </ul>
                        </div>
                        <div class="row">
                            <div class="col-md-2" id="">
                                <label for="exampleInputuname">{{ translate('Longitude') }}</label>
                            </div>
                            <div class="col-md-10" id="">
                                <input type="text" class="form-control mb-3" id="longitude" name="delivery_pickup_longitude" readonly="" value="{{ $shop->delivery_pickup_longitude }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-2" id="">
                                <label for="exampleInputuname">{{ translate('Latitude') }}</label>
                            </div>
                            <div class="col-md-10" id="">
                                <input type="text" class="form-control mb-3" id="latitude" name="delivery_pickup_latitude" readonly="" value="{{ $shop->delivery_pickup_latitude }}">
                            </div>
                        </div>
                    @else
                        <div class="row">
                            <div class="col-md-2" id="">
                                <label for="exampleInputuname">{{ translate('Longitude') }}</label>
                            </div>
                            <div class="col-md-10" id="">
                                <input type="text" class="form-control mb-3" id="longitude" name="delivery_pickup_longitude" value="{{ $shop->delivery_pickup_longitude }}">
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-md-2" id="">
                                <label for="exampleInputuname">{{ translate('Latitude') }}</label>
                            </div>
                            <div class="col-md-10" id="">
                                <input type="text" class="form-control mb-3" id="latitude" name="delivery_pickup_latitude" value="{{ $shop->delivery_pickup_latitude }}">
                            </div>
                        </div>
                    @endif

                    <div class="form-group mb-0 text-right">
                        <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                    </div>
                </form>
            </div>
        </div>
    @endif


    {{-- Category Settings --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">Category</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf

                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Category</label>
                    <div class="col-md-10">
                        <select class="form-control" name="categories[]" multiple>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    @if(!empty($shop_categories) && in_array($category->id, $shop_categories))
                                        selected
                                    @endif
                                >{{ $category->enName }}</option>
                            @endforeach
                        </select>

                    </div>
                </div>

                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Localisation Settings --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">Localisation</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf

                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Latitude</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="latitude" name="latitude" value="{{ $shop->latitude }}">
                        <small class="text-muted">latitude</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Longitude</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="longitude" name="longitude" value="{{ $shop->longitude }}">
                        <small class="text-muted">longitude</small>
                    </div>
                </div>

                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Branches Settings --}}

    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">Branches</h5>
        </div>
        <div class="card-body">
            <table class="table  mb-0">
                <thead>
                    <tr>
                        <th>#ID</th>
                        <th>City</th>
                        <th>Arabic City</th>
                        <th>Name</th>
                        <th>Address</th>
                        <th>Arabic Address</th>
                        <th>QR code</th>
                        <th>GPS</th>
                        <th>Action</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($branches as $key=>$branch)
                        <tr>
                            <td>{{$branch->id}}</td>
                            <td>{{$branch->city_name}}</td>
                            <td>{{$branch->city_name_ar}}</td>
                            <td>{{$branch->name_ar}}</td>
                            <td>{{$branch->address}}</td>
                            <td>{{$branch->address_ar}}</td>
                            <td>
                                <img
                                    class = "print_img"
                                    id = "print_img_header2"
                                    width="50%"
                                    src="https://qrcode.tec-it.com/API/QRCode?data={{ url("/").'/id/shop/'. $branch->slog }}" />
                            </td>
                            <td>Lat: {{$branch->latitude}}, Log: {{$branch->longitude}}</td>
                            <td>
                                <button onclick="editBranch('{{$branch->id}}');" class="btn btn-primary">{{ translate('Edit') }}</button>
                                <a href="{{ route('delete_branch', $branch->id) }}" class="btn btn-primary">{{ translate('Delete') }}</a>
                                <a style="color: #FFFFFF" onclick="printDiv('{{ $branch->slog }}')" class="btn btn-primary">{{ translate('Print') }}</a>
                            </td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
            <br/>
            <h5>Add Branch</h5>
            <form class="" action="{{ route('add_branch') }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="shop_id" value="{{$shop->id}}">
                @csrf
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Country') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <select class="form-control" name="country_id" id="country_id" onchange="getCities('city_id2')">
                            <option value="" >{{ translate("choose a Country") }}</option>
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}" <?php if($country->id ==  @$shop->city->state_id) echo "selected" ?>>{{ $country->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('City') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <select class="form-control" name="city_id" id="city_id2">
                            <option value="" hidden selected>{{ translate("choose a City") }}</option>
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}" {{($city->id == $shop->city_id)?'selected':'' }}>{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Name</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="name" name="name" />
                        <small class="text-muted">Name</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Arabic Name</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="Arabic name" name="name_ar" />
                        <small class="text-muted">Arabic Name</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Address</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="address" name="address" />
                        <small class="text-muted">Address</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Arabic Address</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="Arabic address" name="address_ar" />
                        <small class="text-muted">Arabic Address</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Web Site</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="Web url" name="website" />
                        <small class="text-muted">Web site</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Latitude</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="latitude" name="latitude" />
                        <small class="text-muted">latitude</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Longitude</label>
                    <div class="col-md-10">
                        <input type="text" class="form-control" placeholder="longitude" name="longitude" />
                        <small class="text-muted">longitude</small>
                    </div>
                </div>

                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Times Settings --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">Opening times</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf

                <label class="col-form-label">Monday</label>
                <div class="row mb-3">
                    <div class="col row">
                        <label class="col-md-2 col-form-label">From</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="mo_from" placeholder="01" value="{{ $shop->monday_from }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">AM</small>-->

                    </div>
                    <div class="col row">
                        <label class="col-md-2 col-form-label">To</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="mo_to"  placeholder="12" value="{{ $shop->monday_to }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">PM</small>-->
                    </div>
                </div>
                <label class="col-form-label">Tuesday</label>
                <div class="row mb-3">
                    <div class="col row">
                        <label class="col-md-2 col-form-label">From</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="tu_from" placeholder="01" value="{{ $shop->tuesday_from }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">AM</small>-->

                    </div>
                    <div class="col row">
                        <label class="col-md-2 col-form-label">To</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="tu_to"  placeholder="12" value="{{ $shop->tuesday_to }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">PM</small>-->
                    </div>

                </div>
                <label class="col-form-label">Wednesday</label>
                <div class="row mb-3">
                    <div class="col row">
                        <label class="col-md-2 col-form-label">From</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="we_from" placeholder="01" value="{{ $shop->wednesday_from }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">AM</small>-->

                    </div>
                    <div class="col row">
                        <label class="col-md-2 col-form-label">To</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="we_to"  placeholder="12" value="{{ $shop->wednesday_to }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">PM</small>-->
                    </div>
                </div>
                <label class="col-form-label">Thursday</label>
                <div class="row mb-3">
                    <div class="col row">
                        <label class="col-md-2 col-form-label">From</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="th_from" placeholder="01" value="{{ $shop->thursday_from }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">AM</small>-->

                    </div>
                    <div class="col row">
                        <label class="col-md-2 col-form-label">To</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="th_to"  placeholder="12" value="{{ $shop->thursday_to }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">PM</small>-->
                    </div>

                </div>
                <label class="col-form-label">Friday</label>
                <div class="row mb-3">
                    <div class="col row">
                        <label class="col-md-2 col-form-label">From</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="fr_from" placeholder="01" value="{{ $shop->friday_from }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">AM</small>-->

                    </div>
                    <div class="col row">
                        <label class="col-md-2 col-form-label">To</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="fr_to"  placeholder="12" value="{{ $shop->friday_to }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">PM</small>-->
                    </div>
                </div>
                <label class="col-form-label">Saturday</label>
                <div class="row mb-3">
                    <div class="col row">
                        <label class="col-md-2 col-form-label">From</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="sa_from" placeholder="01" value="{{ $shop->saturday_from }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">AM</small>-->

                    </div>
                    <div class="col row">
                        <label class="col-md-2 col-form-label">To</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="sa_to"  placeholder="12" value="{{ $shop->saturday_to }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">PM</small>-->
                    </div>

                </div>
                <label class="col-form-label">Sunday</label>
                <div class="row mb-3">
                    <div class="col row">
                        <label class="col-md-2 col-form-label">From</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="su_from" placeholder="01" value="{{ $shop->sunday_from }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">AM</small>-->

                    </div>
                    <div class="col row">
                        <label class="col-md-2 col-form-label">To</label>
                        <div class="col-md-8">
                            <input type="time" min="1" max="12" class="form-control" name="su_to"  placeholder="12" value="{{ $shop->sunday_to }}" >
                        </div>
                        <!--<small class="col-md-2 text-muted">PM</small>-->
                    </div>

                </div>

                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Options Settings --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">Options</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf

                <div class="row mb-3">
                    <div class="row col-md-6">
                        <label class="col-md-2 col-form-label">
                            <img src="{{ asset('/public/wifi.png') }}" width="25px" />
                        </label>
                        <div class="col-md-10">
                            <select class="form-control" name="wifi">
                                <option value="0"
                                    @if ($shop->wifi == 0)
                                        selected
                                    @endif
                                >NO</option>
                                <option value="1"
                                    @if ($shop->wifi == 1)
                                        selected
                                    @endif
                                >YES</option>
                            </select>
                        </div>
                    </div>
                    <div class="row col-md-6">
                        <label class="col-md-2 col-form-label">
                            <img src="{{ asset('/public/parking.png') }}" width="25px" />
                        </label>
                        <div class="col-md-10">
                            <select class="form-control" name="parck">
                                <option value="0"
                                    @if ($shop->parck == 0)
                                        selected
                                    @endif
                                >NO</option>
                                <option value="1"
                                    @if ($shop->parck == 1)
                                        selected
                                    @endif
                                >YES</option>
                            </select>
                        </div>
                    </div>
                </div>

                <div class="row mb-3">
                    <div class="row col-md-6">
                        <label class="col-md-2 col-form-label">
                            <img src="{{ asset('/public/t-shirt.png') }}" width="25px" />
                        </label>
                        <div class="col-md-10">
                            <select class="form-control" name="serverman">
                                <option value="0"
                                    @if ($shop->server == 0)
                                        selected
                                    @endif
                                >NO</option>
                                <option value="1"
                                    @if ($shop->server == 1)
                                        selected
                                    @endif
                                >YES</option>
                            </select>
                        </div>
                    </div>
                    <div class="row col-md-6">
                        <label class="col-md-2 col-form-label">
                            <img src="{{ asset('/public/no_children.png') }}" width="25px" />
                        </label>
                        <div class="col-md-10">
                            <select class="form-control" name="kids">
                                <option value="0"
                                    @if ($shop->kids == 0)
                                        selected
                                    @endif
                                >NO</option>
                                <option value="1"
                                    @if ($shop->kids == 1)
                                        selected
                                    @endif
                                >YES</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="row mb-3">
                    <div class="row col-md-6">
                        <label class="col-md-2 col-form-label">
                            <img src="{{ asset('/public/bag.png') }}" width="25px" />
                        </label>
                        <div class="col-md-10">
                            <select class="form-control" name="bag">
                                <option value="0"
                                    @if ($shop->bag == 0)
                                        selected
                                    @endif
                                >NO</option>
                                <option value="1"
                                    @if ($shop->bag == 1)
                                        selected
                                    @endif
                                >YES</option>
                            </select>
                        </div>
                    </div>
                </div>
                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Menu Settings --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">Menu</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf

                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Images</label>
                    <div class="col-md-10">
                        <input type="file" class="form-control" name="menu[]" placeholder="address" multiple />
                    </div>
                    <br/><br/>
                    <div class="file-preview box sm">
                        <?php
                            if($shop->menu != null){
                                $images = explode("|", $shop->menu);
                                foreach($images as $img){
                                    echo "<img src='".asset("/public/done-i.png")."' width='300px' /> ";
                                }
                            }

                        ?>
                    </div>
                </div>


                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

        {{-- Banner Settings --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Banner Settings') }}</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf

                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('Banners') }} (1500x450)</label>
                    <div class="col-md-10">
                        <div class="input-group" data-toggle="aizuploader" data-type="image" data-multiple="true">
                            <div class="input-group-prepend">
                                <div class="input-group-text bg-soft-secondary font-weight-medium">{{ translate('Browse')}}</div>
                            </div>
                            <div class="form-control file-amount">{{ translate('Choose File') }}</div>
                            <input type="hidden" name="sliders" value="{{ $shop->sliders }}" class="selected-files">
                        </div>
                        <div class="file-preview box sm">
                        </div>
                        <small class="text-muted">{{ translate('We had to limit height to maintian consistancy. In some device both side of the banner might be cropped for height limitation.') }}</small>
                    </div>
                </div>

                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Default Image Settings --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">Default Image</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf

                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Image</label>
                    <div class="col-md-10">
                        <input type="file" class="form-control" name="default_image" placeholder="address" multiple />
                    </div>
                    <br/><br/>
                    <div class="file-preview box sm">
                            <img src='{{ static_asset($shop->default_image) }}' width='300px' />
                    </div>
                </div>


                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>

    {{-- Social Media Link --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Social Media Link') }}</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('seller.shop.update') }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="shop_id" value="{{ $shop->id }}">
                @csrf
                <div class="form-box-content p-3">
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Facebook') }}</label>
                        <div class="col-md-10">
                            <input type="text" class="form-control" placeholder="{{ translate('Facebook')}}" name="facebook" value="{{ $shop->facebook }}">
                            <small class="text-muted">{{ translate('Insert link with https ') }}</small>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Instagram') }}</label>
                        <div class="col-md-10">
                            <input type="text" class="form-control" placeholder="{{ translate('Instagram')}}" name="instagram" value="{{ $shop->instagram }}">
                            <small class="text-muted">{{ translate('Insert link with https ') }}</small>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Twitter') }}</label>
                        <div class="col-md-10">
                            <input type="text" class="form-control" placeholder="{{ translate('Twitter')}}" name="twitter" value="{{ $shop->twitter }}">
                            <small class="text-muted">{{ translate('Insert link with https ') }}</small>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Google') }}</label>
                        <div class="col-md-10">
                            <input type="text" class="form-control" placeholder="{{ translate('Google')}}" name="google" value="{{ $shop->google }}">
                            <small class="text-muted">{{ translate('Insert link with https ') }}</small>
                        </div>
                    </div>
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Youtube') }}</label>
                        <div class="col-md-10">
                            <input type="text" class="form-control" placeholder="{{ translate('Youtube')}}" name="youtube" value="{{ $shop->youtube }}">
                            <small class="text-muted">{{ translate('Insert link with https ') }}</small>
                        </div>
                    </div>
                </div>
                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>


    <!-------------------Saim's work----------------------->
    {{-- Terms and Conditions --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Terms And Conditions / ENGLISH') }}</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf
                <div class="form-box-content p-3">
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Terms And Conditions') }}</label>
                        <div class="col-md-10">
                            <textarea
                                    class="aiz-text-editor form-control"
                                    placeholder="{{translate('Content..')}}"
                                    data-buttons='[["font", ["bold", "underline", "italic", "clear"]],["para", ["ul", "ol", "paragraph"]],["style", ["style"]],["color", ["color"]],["table", ["table"]],["insert", ["link", "picture", "video"]],["view", ["fullscreen", "codeview", "undo", "redo"]]]'
                                    data-min-height="300"
                                    name="terms_en"
                                    required
                            >{{ $shop->terms_en }}</textarea>
                            <!--<small class="text-muted">{{ translate('Insert link with https ') }}</small>-->
                        </div>
                    </div>
                </div>
                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>
    <!----------end Saim's work--------->


     <!-------------------ahmed's work----------------------->
    {{-- Terms and Conditions --}}
    <div class="card">
        <div class="card-header">
            <h5 class="mb-0 h6">{{ translate('Terms And Conditions / ARABIC') }}</h5>
        </div>
        <div class="card-body">
            <form class="" action="{{ route('shops.update', $shop->id) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="_method" value="PATCH">
                @csrf
                <div class="form-box-content p-3">
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Terms And Conditions') }}</label>
                        <div class="col-md-10">
                            <textarea
                                    class="aiz-text-editor form-control"
                                    placeholder="{{translate('Content..')}}"
                                    data-buttons='[["font", ["bold", "underline", "italic", "clear"]],["para", ["ul", "ol", "paragraph"]],["style", ["style"]],["color", ["color"]],["table", ["table"]],["insert", ["link", "picture", "video"]],["view", ["fullscreen", "codeview", "undo", "redo"]]]'
                                    data-min-height="300"
                                    name="terms_ar"
                                    required
                            >{{ $shop->terms_ar }}</textarea>
                            <!--<small class="text-muted">{{ translate('Insert link with https ') }}</small>-->
                        </div>
                    </div>
                </div>
                <div class="form-group mb-0 text-right">
                    <button type="submit" class="btn btn-sm btn-primary">{{translate('Save')}}</button>
                </div>
            </form>
        </div>
    </div>
    <!----------end Ahmed's work--------->

    {{-- edit brande modal start --}}
    <div class="modal fade" id="edit_branch">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title h6">{{translate('Edit Branch')}}</h5>
                    <button type="button" class="close" data-dismiss="modal"></button>
                </div>
                <form class=""  id="edited_branch_form" action="{{ route('edit_branch',0) }}" method="POST" enctype="multipart/form-data">
                <input type="hidden" name="edited_branch_id" value="">
                @csrf
                    <div class="row mb-3">
                        <label class="col-md-2 col-form-label">{{ translate('Country') }}<span class="text-danger text-danger">*</span></label>
                        <div class="col-md-10">
                            <select class="form-control" name="country_id" id="country_id" onchange="getCities('edited_branch_city')">
                                <option value="" >{{ translate("choose a Country") }}</option>
                                @foreach ($countries as $country)
                                    <option value="{{ $country->id }}" <?php if($country->id ==  @$shop->city->state_id) echo "selected" ?>>{{ $country->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">{{ translate('City') }}<span class="text-danger text-danger">*</span></label>
                    <div class="col-md-10">
                        <select id="edited_branch_city" class="form-control" name="city_id" required>
                            <option hidden selected>{{ translate("choose a City") }}</option>
                            @foreach ($cities as $city)
                                <option value="{{ $city->id }}">{{ $city->name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Name</label>
                    <div class="col-md-10">
                        <input type="text" id="edited_branch_name" class="form-control" placeholder="name" name="name" />
                        <small class="text-muted">Name</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Arabic Name</label>
                    <div class="col-md-10">
                        <input type="text" id = 'edited_branch_name_ar' class="form-control" placeholder="Arabic name" name="name_ar" />
                        <small class="text-muted">Arabic Name</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Address</label>
                    <div class="col-md-10">
                        <input type="text" id='edited_branch_address' class="form-control" placeholder="address" name="address" />
                        <small class="text-muted">Address</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Arabic Address</label>
                    <div class="col-md-10">
                        <input type="text" id='edited_branch_address_ar' class="form-control" placeholder="Arabic address" name="address_ar" />
                        <small class="text-muted">Arabic Address</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Web Site</label>
                    <div class="col-md-10">
                        <input id='edited_branch_website' type="text" class="form-control" placeholder="Web url" name="website" />
                        <small class="text-muted">Web site</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Latitude</label>
                    <div class="col-md-10">
                        <input id='edited_branch_latitude' type="text" class="form-control" placeholder="latitude" name="latitude" />
                        <small class="text-muted">latitude</small>
                    </div>
                </div>
                <div class="row mb-3">
                    <label class="col-md-2 col-form-label">Longitude</label>
                    <div class="col-md-10">
                        <input  id='edited_branch_longitude' type="text" class="form-control" placeholder="longitude" name="longitude" />
                        <small class="text-muted">longitude</small>
                    </div>
                </div>

                <div class="modal-footer">
                    <button type="button" class="btn btn-light" data-dismiss="modal">{{translate('Cancel')}}</button>
                    <button type="submit" class="btn btn-primary">{{translate('Proceed!')}}</button>
                </div>
                </form>
            </div>
        </div>
    </div>
    {{-- edit brande modal end--}}
        <img class="print_img"  style="display:none"  id="print_img_header1" width="66%" class=""
        src="data:image/png;base64,"  />
        <img   style="display:none"  class="print_img" id="print_img_header22" width="66%" class=""
        src="data:image/png;base64," />
        <img  style="display:none"  class="print_img" id="print_img_header3" width="66%" class=""
            src="data:image/png;base64," />
    @endsection

@section('script')

    @if (get_setting('google_map') == 1)

        @include('frontend.partials.google_map')

    @endif

    <script>
        function printDiv(branch_slog) {
            var a = window.open('', '', 'height=500, width=500');
            var print_img_header2 = $("<img>", {
                src: 'https://qrcode.tec-it.com/API/QRCode?data=' +
                window.location.origin + '/id/shop/' + branch_slog,
                alt: "Image description",
                class: 'print_img',
                id:'print_img_header2'

            });

            var print_img_header1 = $("#print_img_header1").clone();
            var print_img_header22 = $("#print_img_header22").clone();
            var print_img_header3 = $("#print_img_header3").clone();
            print_img_header1.css("display","block");
            print_img_header22.css("display","block");
            print_img_header3.css("display","block");
            var parent = $("<div id='parent'></div>");
            var center = $('<center></center>')
            var card_qr = $('<div id="card_qr" class="mb-3" ></div>')

            parent.append(print_img_header1)
            parent.append(print_img_header22)
            parent.append(print_img_header2)
            parent.append(print_img_header3)

            center.append(parent)
            card_qr.append(center)
            var divContents = card_qr.html();
            a.document.write('<html><style> html, body { width: 55mm; height: 9mm; } </style><style> .print_img { width:100%; } </style>');
            a.document.write('<style> #print_img_header2{ height: 35mm;width: 35mm;} </style>')
            a.document.write('<body style="display: flex;flex-direction: column;box-sizing: border-box;padding-top:0px;border:1px solid black;margin :0" >');
            a.document.write(divContents);
            a.document.write('</body></html>');
            a.document.close();
            setTimeout(() => {
                a.print();
            }, 1000);
        }
        function editBranch(branchId)
        {
            const branchesDOM = document.getElementById("branches");
            const branches = JSON.parse(branchesDOM.dataset.obj);
            const branch = branches.find(b => b.id == branchId);
            if(branch){
                $('#edited_branch_id').val(branch.id);
                $('#edited_branch_name').val(branch.name);
                $('#edited_branch_name_ar').val(branch.name_ar);
                $('#edited_branch_address').val(branch.address);
                $('#edited_branch_address_ar').val(branch.address_ar);
                $('#edited_branch_website').val(branch.website);
                $('#edited_branch_latitude').val(branch.latitude);
                $('#edited_branch_longitude').val(branch.longitude);
                var url = `${window.location.origin}/edit_branch/${branch.id}`;
                $('#edited_branch_form').attr("action", url);
                $('#edited_branch_city').val(branch.city_id);
                $('#edit_branch').modal('show', {backdrop: 'static'});
            }else{
                console.log(`Branch with Id ${branchId} not found`);
            }

        }

        function getCities(id_element) {
            let id = $('#country_id').val();
            $.ajax({
                       headers: {
                           'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                       },
                       url: "/seller/getCities/"+id,
                       type: 'get',

                       cache: false,
                       contentType: false,
                       processData: false,
                       success: function (response) {
                           let html='<option></option>';
                           for (var i = 0; i < response.length; i++) {
                               html +=`<option value=\"${response[i].id}\">${response[i].name}</option>`;
                           }
                           $('#'+id_element).empty();
                           $('#'+id_element).append(html);
                       }
                   });
            console.log('dsa')

        }

    </script>


@endsection
