    ...
    public function getList(Request $request)
    {
        ...
        if ($list == "online" || $list == "both") {
            ...
            if(get_setting('Hyperpay_visapayment') == 1){
                $payment_type = array();
                $payment_type['payment_type'] = 'VISA';
                $payment_type['payment_type_key'] = 'VISA';
                $payment_type['image'] = static_asset('assets/img/cards/VISA.png');
                $payment_type['name'] = translate("VISA");
                $payment_type['title'] = translate("Checkout with VISA");
                $payment_type['offline_payment_id'] = 0;
                $payment_type['details'] = "";
                if ($mode == 'wallet') {
                    $payment_type['title'] = translate("Recharge with VISA");
                }

                $payment_types[] = $payment_type;

                $payment_type = array();
                $payment_type['payment_type'] = 'MASTER';
                $payment_type['payment_type_key'] = 'MASTER';
                $payment_type['image'] = static_asset('assets/img/cards/MASTER.png');
                $payment_type['name'] = translate("MASTER");
                $payment_type['title'] = translate("Checkout with MASTER");
                $payment_type['offline_payment_id'] = 0;
                $payment_type['details'] = "";
                if ($mode == 'wallet') {
                    $payment_type['title'] = translate("Recharge with MASTER");
                }

                $payment_types[] = $payment_type;

                $payment_type = array();
                $payment_type['payment_type'] = 'AMEX';
                $payment_type['payment_type_key'] = 'AMEX';
                $payment_type['image'] = static_asset('assets/img/cards/AMEX.png');
                $payment_type['name'] = translate("AMEX");
                $payment_type['title'] = translate("Checkout with AMEX");
                $payment_type['offline_payment_id'] = 0;
                $payment_type['details'] = "";
                if ($mode == 'wallet') {
                    $payment_type['title'] = translate("Recharge with AMEX");
                }

                $payment_types[] = $payment_type;

                $payment_type = array();
                $payment_type['payment_type'] = 'STC_PAY';
                $payment_type['payment_type_key'] = 'STC_PAY';
                $payment_type['image'] = static_asset('assets/img/cards/STC_PAY.png');
                $payment_type['name'] = translate("STC_PAY");
                $payment_type['title'] = translate("Checkout with STC_PAY");
                $payment_type['offline_payment_id'] = 0;
                $payment_type['details'] = "";
                if ($mode == 'wallet') {
                    $payment_type['title'] = "Recharge with STC_PAY";
                }
            ...
            if(get_setting('Hyperpay_payment') == 1){
                $payment_type = array();
                $payment_type['payment_type'] = 'MADA';
                $payment_type['payment_type_key'] = 'MADA';
                $payment_type['image'] = static_asset('assets/img/cards/MADA.png');
                $payment_type['name'] = translate("MADA");
                $payment_type['title'] = translate("Checkout with MADA");
                $payment_type['offline_payment_id'] = 0;
                $payment_type['details'] = "";
                if ($mode == 'wallet') {
                    $payment_type['title'] = translate("Recharge with MADA");
                }
        ...
        if ($mode != 'wallet' && $mode != 'seller_package' && $list != "offline") {
            if (get_setting('wallet_system') == 1) {
                ...
                $payment_type['name'] = translate("Wallet");
