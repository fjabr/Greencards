...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            ...
            <form class="" id=""
                action="{{ route('customers.export_customers_request') }}"
                method="POST">
                @csrf
            ...
            <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="lg">{{ translate('Expired on') }}</th>
                                ...
                                <td>
                                    @if (auth()->user()->can('edit_customer'))
                                        <a href="{{ route('customers.edit', encrypt($user->id)) }}">
                                            @if ($user->banned == 1)
                                                <i class="fa fa-ban text-danger" aria-hidden="true">{{ $user->name }}</i>
                                            @endif {{ $user->name }}
                                        </a>
                                    @else
                                        @if ($user->banned == 1)
