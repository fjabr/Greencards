...
Route::group(['prefix' => 'v2/auth', 'middleware' => ['app_language']], function() {
    ...
    Route::middleware('auth:sanctum')->group(function () {
        ...
        Route::post('delete', 'App\Http\Controllers\Api\V2\AuthController@delete');
...
Route::group(['prefix' => 'by-bill', 'middleware' => ['auth:sanctum']], function() {
    Route::get('/get-my-bill-url', [InvoiceController::class, 'getMyBillPageUrl'])->name('byBill.getMyBillPageUrl');
});
Route::group(['prefix' => 'send-gift', 'middleware' => ['auth:sanctum']], function() {
    Route::get('/get-send-gift-url', [MobileController::class, 'getSendGiftUrl']);
});

Route::group(['prefix' => 'v2', 'middleware' => ['app_language']], function() {

    Route::get('customer-packages', 'App\Http\Controllers\Api\V2\CustomerPackageController@customer_packages_list')->name('customer.packages');
    Route::get('banners/bigSaleBanner', 'App\Http\Controllers\Api\V2\SliderController@getBigSaleBanner')->name('slider.getBigSaleBanner');
    Route::get('banners/productLessThanPrice', 'App\Http\Controllers\Api\V2\SliderController@ProductLessThanPrice')->name('slider.getBigSaleBanner');
    Route::get('bigSale', [MobileController::class, 'getBigSale'])->name("mobile.bigSale");
    Route::get('subscriptions/subscription-link', 'App\Http\Controllers\Api\V2\SubscriptionController@getSubscriptionLink')->name('subscriptions.getSubscriptionPageUrl')->middleware('auth:sanctum');
    ...
    Route::group(['middleware' => ['app_user_unbanned','app_language']], function() {
    ...
    Route::get('sliders', 'App\Http\Controllers\Api\V2\SliderController@sliders_v2');
    Route::get('recommendedSellers', 'App\Http\Controllers\Api\V2\SliderController@recommendedSellers');
    Route::get('sliders_v2', 'App\Http\Controllers\Api\V2\SliderController@sliders_v2');
    ...
    Route::any('hyperpay/payment/pay', 'Api\V2\HyperpayController@pay')->name('api.hyperpay.pay');
    Route::any('hyperpay/payment/response', 'Api\V2\HyperpayController@response')->name('api.hyperpay.response');
    Route::any('hyperpay/payment/visaresponse', 'Api\V2\HyperpayController@visaresponse')->name('api.hyperpay.visaresponse');
 ...
 Route::prefix('mobile')->group(function () {
    ...
    {
        Route::post('signup', [MobileController::class, 'signupWithPhoneNumber'])->name("auth.mobile.signup");
        Route::post('login', [MobileController::class, 'loginWithPhoneNumber'])->name("auth.mobile.login");
        Route::post('confirm-code', [MobileController::class, 'confirmCode'])->name("auth.mobile.confirm-code");
        Route::post('resend-code', [MobileController::class, 'resendCode'])->name("auth.mobile.resend-code");
    });
    Route::get('offline-payment/instructions', "App\Http\Controllers\MobileController@getOfflinePayment");
    Route::get('categories/{package_id}', "App\Http\Controllers\MobileController@getCategories");
    Route::get('customer_package', "App\Http\Controllers\MobileController@getPackages");
    Route::post('signup', "App\Http\Controllers\MobileController@signup");
    Route::post('login', "App\Http\Controllers\MobileController@login");
    Route::post('login_google', "App\Http\Controllers\MobileController@loginGoogle");
    Route::post('resetPassword', "App\Http\Controllers\MobileController@resetPasssword");
    Route::post('resetPassword_2', "App\Http\Controllers\MobileController@resetPasssword_V2");
    Route::get('countries', "App\Http\Controllers\MobileController@getCountries");
    Route::post('payment', "App\Http\Controllers\MobileController@payment");

    Route::post('update_payment', "App\Http\Controllers\MobileController@UpdatePayment");

    Route::get('shops_by_city/{city}', "App\Http\Controllers\MobileController@getShopsByCity");
    Route::get('shops/{category}', "App\Http\Controllers\MobileController@getShops");
    Route::get('allShop', "App\Http\Controllers\MobileController@getAllShopsLocations");
    Route::post('shops_by_location', "App\Http\Controllers\MobileController@getShopsByLocation");
    Route::post('shops_by_name', "App\Http\Controllers\MobileController@getShopsByName");
    Route::get('shop/{id}', "App\Http\Controllers\MobileController@getShop");
    Route::get('offers/{id}/user/{id_user}', "App\Http\Controllers\MobileController@getOffers");
    Route::post('scan_offer', "App\Http\Controllers\MobileController@scanOffer");
    Route::post('approve_scan_offer', "App\Http\Controllers\MobileController@approveScanOffer");
    Route::post('cancel_scan_offer', "MobileController@CancelScanOffer");

    Route::get('orders/{id}', "App\Http\Controllers\MobileController@getOrders");
    Route::get('package/{id}', "App\Http\Controllers\MobileController@getPackage");
    Route::get('user/{id}', "App\Http\Controllers\MobileController@getUser");

    Route::get('family/{id}', "App\Http\Controllers\MobileController@getFamily");
    Route::post('add_user_family', "App\Http\Controllers\MobileController@addFamily")->middleware('auth:sanctum');
    Route::post('update_user_family', "App\Http\Controllers\MobileController@updateFamily");
    Route::post('delete_user_family', "App\Http\Controllers\MobileController@deleteFamily");

    Route::get('products_points', "App\Http\Controllers\MobileController@getProducts");

    Route::get('wishlists/{id}', "App\Http\Controllers\MobileController@getWishlists");
    Route::post('add_wishlists', "App\Http\Controllers\MobileController@AddToWishlists");
    Route::get('delete_wishlists/update_user{id}', "App\Http\Controllers\MobileController@deleteFromWishLists");

    Route::get('notifications/{id}', "App\Http\Controllers\MobileController@getNotifications");

    Route::get('shops_by_type_offers/{type}', "App\Http\Controllers\MobileController@ShopsByTypeOffers");

    Route::get('fatoora/{src}', "App\Http\Controllers\MobileController@getInvoice");

    Route::get('wishlists_products/{id}', 'Api\V2\WishlistController@index');

    Route::post('', 'App\Http\Controllers\MobileController@updateUser')->middleware('auth:sanctum');
    Route::post('update_location', 'App\Http\Controllers\MobileController@updateLocationUser');
    Route::post('update_pwd', 'App\Http\Controllers\MobileController@updatePwd');

    Route::get('getCoupons/{id}', 'App\Http\Controllers\MobileController@getCoupons');

    Route::get('getBranches/{id}', 'App\Http\Controllers\MobileController@getBranches');

    /**
     * saim's work
     */
    Route::get('get_terms/{id}', 'App\Http\Controllers\MobileController@getTerms');
    Route::post('get-nearby-shops', "App\Http\Controllers\MobileController@getNearbyShops");
    Route::post('notification_seen', "App\Http\Controllers\MobileController@seenNotification");
    Route::post('notification_seen', "MobileController@seenNotification");
    Route::post('hyperpay', "App\Http\Controllers\MobileController@hyperpay");
    // end Saim's work

    // MISLAM
    Route::post('offline-customer-package-payment', "App\Http\Controllers\CustomerOfflinePackageController@offline_customer_package_payment");
    Route::post('claim-subscription-coupon', 'App\Http\Controllers\MobileController@claimSubscriptionCoupon');



    // Route::get("send_sms", "MobileController@sendSmsTest");

});

 Route::post('scale', function(Request $request){
 ...
 */
/*
    foreach (Upload::all() as $value) {
        $value->external_link = env("AWS_URL").$value->file_name;
        $value->save();
    }

    return my_asset("images/i2YvnPypaLV7m1XPtPHnUJ1SQ5xwG0u9qZ5TTGiI.png"); */

    $sumPayment = CustomerPackagePayment::where("approval", "0")->where("created_at",'<','2023-01-01')->sum("amount");
    return $sumPayment;
 });

 Route::post('blance_package_with_user_member_number', function(Request $request){

    $users = User::where("user_type","customer")->get();
    foreach ($users as $user) {
        if(!empty($user->parant_id ) && $user->parant_id!=null){
            $parent = User::where("id",$user->parant_id)->first();
            if(!empty($parent)){
                $user->end_sub_date = $parent->end_sub_date;
                $user->start_sub_date = $parent->start_sub_date;
                $user->duration = $parent->duration;
            }
            $user->nb_members = 0;
        }else{
            $package = CustomerPackage::where("id",$user->customer_package_id)->first();
            if(!empty($package)){
                $user->nb_members = $package->nb_members-1;
                $familyMembersCount = User::where("parant_id",$user->id)->count();
                $user->nb_members = $user->nb_members-$familyMembersCount;
