...
Route::controller(HomeController::class)->group(function () {
    Route::get('/customers/links/subscription/success', 'invitationLinkRegistrationSuccess')->name('user.links.register.success');
    Route::get('/customers/links/subscription/{id}', 'invitationByLinkRegistration')->name('user.links.invitationByLink');
    Route::post('/customers/links/subscription/{id}', 'invitationLinkRegistration')->name('user.links.register');
    ...
    Route::get('/users/registration/{package?}', 'registration')->name('user.registration');
    ...
    Route::get('/customer-packages/{package?}', 'premium_package_index')->name('customer_packages_list_show');
// this route added by hassane hamarrass  for showng  all packages that we have
    Route::get('/packages', 'subscription_package')->name('subscription_package');
    ...
    Route::get('/send-gift/success', 'invitationLinkRegistrationSuccess')->name('user.send.gift.success');
    Route::get('/send-gift/{id}', 'sendGiftForm')->name('user.send.gift.form');
    Route::post('/send-gift', 'sendGiftRegistration')->name('user.send.gift.registration');

});

Route::get('/subscription-payment',  [\App\Http\Controllers\PaymentController::class, 'subscriptionPayment'])->name("subscription-payment");
Route::get('/subscription-checkout/{id}',  [\App\Http\Controllers\PaymentController::class, 'subscriptionCheckout']);
Route::get('/subscription-complete',  [\App\Http\Controllers\PaymentController::class, 'subscriptionComplete']);

Route::get('/order-payment',  [\App\Http\Controllers\OrderPaymentController::class, 'orderPayment'])->name("order-payment");
Route::get('/order-checkout/{id}',  [\App\Http\Controllers\OrderPaymentController::class, 'orderCheckout'])->name("order-checkout");
Route::get('/order-complete',  [\App\Http\Controllers\OrderPaymentController::class, 'orderComplete'])->name("order-complete");



Route::resource('/api/mobile/user-package',  UserPackageController::class);
...
// Cart
Route::controller(CarftController::class)->group(function () {
...
Route::resource('subscribers', SubscriberController::class);
    ...
    Route::get('/frontend/hyperpay', [HyperpayController::class,'mada'])->name('mada');
    Route::get('/frontend/hyperpay/visa', [HyperpayController::class,'visa'])->name('visa');
    Route::get('/frontend/thankyou', [ThankyouController::class,'thankyou'])->name('thankyou');
    Route::any('/hyperpay/payment/visaresponse', [HyperpayController::class,'visaresponse'])->name('visaresponse');
    Route::any('/hyperpay/payment/stcpay', [HyperpayController::class,'stcpay'])->name('stcpay');
    Route::any('/hyperpay/payment/stcpayresponse', [HyperpayController::class,'stcpayresponse'])->name('stcpayresponse');
    Route::any('/hyperpay/payment/response', [HyperpayController::class,'response'])->name('response');


    Route::controller(HomeController::class)->group(function () {
        Route::get('/id/shop/{shopSlug}', 'seller_subscribtion_gateway')->name('seller_subscribtion_gateway');
        Route::post('/id/shop', 'customerSubscribeFromSellerQR')->name('sellers.customer.subscribe');
    });
...
Route::group(['middleware' => ['user', 'verified', 'unbanned']], function() {
    ...
    Route::post('/send_contract', 'contract')->name('send_contract');
    Route::post('/upload_contract', 'saveContract')->name('upload_contract');
    Route::get('/init_contract/{id}', 'init_contract')->name('init_contract');
    Route::get('/delete_offer/{id}', 'deleteOffer')->name('deleteOffer');
    Route::post('/add_offer', 'addOffer')->name('addOffer');
    Route::get('/edit_offer/{id}', 'editOffer')->name('editOffer');
    Route::get('/offer-statistics/{offer}', 'offerStatistics')->name('OfferStatistics');
    Route::post('/update_offer', 'updateOffer')->name('updateOffer');
...
Route::group(['prefix' => 'checkout'], function() {
    Route::controller(CheckoutController::class)->group(function () {
        Route::post('/apply_coupon_code3', 'apply_coupon_code')->name('checkout.apply_coupon_code3');
    });
});
...
Route::get('my-bill/{id}', [InvoiceController::class, 'myBill'])->name('invoice.myBill');
Route::get('/customer-packages-purchase/{id}', [SubscriberController::class, 'premium_package_index'])->name('subscriptions.customers.packages');
...
Route::group(['middleware' => ['auth']], function() {
    ...
    Route::get('/my-bills', [InvoiceController::class, 'myBills'])->name('bills.myBills');
    Route::get('/bill-details/{id}', [InvoiceController::class, 'bill_details'])->name('bills.bill_details');
    Route::get('/download_bill/{id}', [InvoiceController::class, 'send_bill_via_email'])->name('bills.send_bill_via_email');
    Route::get('/send_bill_via_email/{id}', [InvoiceController::class, 'download_bill'])->name('bills.download_bill');
...
Route::post('/add_branch', [ShopController::class,'addBranch'])->name('add_branch');
Route::post('/edit_branch/{id}', [ShopController::class,'editBranch'])->name('edit_branch');
Route::get('/delete_branch/{id}', [ShopController::class,'deleteBranch'])->name('delete_branch');
...
Route::get('/is-subscribed-user/{user}', function (User $user){
    return [
