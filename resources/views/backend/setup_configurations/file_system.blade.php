...
@section('content')
    ...
    <div class="row">
        ...
        <div class="col-xxl-6">
            <div class="card">
                <div class="card-header">
                    <h3 class="fs-18 mb-0 text-center">{{translate('Fire Base Api Key')}}</h3>
                </div>
                <div class="card-body">
                    <form class="form-horizontal" action="{{ route('payment_method.update') }}" method="POST">
                        <input type="hidden" name="payment_method" value="paypal">
                        @csrf
                        <div class="form-group row">
                            <input type="hidden" name="types[]" value="API_ACCESS_KEY">
                            <div class="col-lg-4">
                                <label class="control-label">{{translate('API_ACCESS_KEY')}}</label>
                            </div>
                            <div class="col-lg-8">
                                <input type="text" class="form-control" name="API_ACCESS_KEY" value="{{  env('API_ACCESS_KEY') }}" placeholder="{{ translate('API_ACCESS_KEY') }}" required>
                            </div>
                        </div>
                    </form>
                </div>
