        ...
        <div class="card-body">
            <table class="table aiz-table mb-0">
                ...
                <tr>
                    ...
                    <th data-breakpoints="lg">{{translate('Categoried')}}</th>
                ...
                @foreach($shops as $key => $shop)
                    <tr>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="uncategorie(this)" value="{{ $shop->id }}" type="checkbox"
                                    <?php if(!empty($shop->categories)) echo "checked";?> >
                                <span class="slider round"></span>
                            </label>
                        </td>
...
@section('script')
    <script type="text/javascript">
        ...
        function uncategorie(el){
            ...
            $.post('{{ route('sellers.uncategorie') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('Operation done successfully') }}');
                }
                else{
                    AIZ.plugins.notify('danger', '{{ translate('Something went wrong') }}');
                }
